...
const BranchDashboard: React.FC<Props> = ({ branch, onBack }) => {
    ...
    const fetchManagers = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/onboard/branches/${branch.id}/managers`, {
                headers: {
                                    ...
                                    <div className="flex flex-col gap-2 border-l border-slate-100 dark:border-slate-800 pl-4 items-end ml-4">
                                        <button
                                            onClick={async () => {
                                                try {
                                                    const token = localStorage.getItem('authToken');
                                                    const res = await fetch(`http://localhost:5000/api/onboard/managers/${manager.id}`, {
                                                        method: 'DELETE',
