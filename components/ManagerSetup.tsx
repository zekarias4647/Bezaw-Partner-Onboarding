...
const ManagerSetup: React.FC<Props> = ({ managers, branches, onChange, onNext, onBack }) => {
  ...
  // Strictly white background and black text for inputs as requested
  const inputStyles = "w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white text-black focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all text-sm font-medium shadow-sm";
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
          ...
          <div key={manager.id} className="bg-white dark:bg-slate-900/60 rounded-[2.5rem] p-8 relative border border-slate-200 dark:border-slate-800 shadow-xl transition-all hover:border-indigo-500/50 group">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4 mb-2">
                ...
                <div className="flex-1">
                  <input
                    ...
                    className="font-black text-xl text-black dark:text-white outline-none border-b border-slate-100 dark:border-slate-800 focus:border-indigo-500 bg-transparent w-full transition-colors uppercase tracking-tight"
                  />
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`${inputStyles} pl-12 font-mono tracking-wider`}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest px-1">Assigned Station</label>
                  <div className="relative">
      ...
      <div className="pt-8 border-t dark:border-slate-800 flex items-center justify-between">
        ...
        <button 
          onClick={onNext} 
          disabled={managers.length === 0 || managers.some(m => !m.name || !m.email)}
          className="px-12 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl shadow-xl shadow-emerald-500/20 disabled:bg-slate-200 dark:disabled:bg-slate-800 transition-all flex items-center gap-2 uppercase text-sm tracking-widest"
        >
