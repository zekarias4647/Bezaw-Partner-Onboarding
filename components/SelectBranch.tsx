    ...
    <div className="flex-1 flex flex-col items-center justify-start animate-fadeIn space-y-8">
      ...
      <div className="w-full max-w-2xl space-y-6">
        <div className="relative group">
          <Search className="absolute left-4 top-4 text-slate-400 dark:text-slate-600 group-focus-within:text-emerald-500 transition-colors" size={20} />
          <input 
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by branch name or location..."
            className="w-full bg-white text-black border border-slate-200 dark:border-slate-800 h-14 pl-12 pr-4 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all"
          />
        </div>
